...
class Draw_binary_classification_results:
    ...
    def roc_curve(self):
        ...
        Linestyle = ["-", "-.", "--", ":", "--"]
        return Drawing2d.plot_matrix_matrix(
            X, Y, Title, xlabel, ylabel, Labels, Linestyle, 0, "equal", False, "", ""
        )
    ...
    def fpn_estimated(self):
        ...
        Linestyle = ["-", "-.", "--", ":", "--"]
        return Drawing2d.plot_matrix_matrix(
            X, Y, Title, xlabel, ylabel, Labels, Linestyle, 2, "equal", False, "", ""
        )
    ...
    def draw_mis_classification(self):
        ...
        for name in model_names:
            ...
            if self.kind == "Validation":
                ...
                zz = pd.DataFrame(
                    np.concatenate([z_res, z_ind], axis=1),
                    columns=["obs", "residue_pearson", "type"],
                )
            elif self.kind == "Prediction":
                ...
                zz = pd.DataFrame(
                    np.concatenate([z_res, z_ind], axis=1),
                    columns=["obs", "prediction_values", "type"],
                )
            else:
                print("Error on kind selection")
            z_list.append(zz)
