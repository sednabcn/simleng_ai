class Simleng_strategies(Data_base):
    ...
    def __init__(self, var, idoc):
        self.var = var
        self.idoc = idoc
        self.data_train, self.data_test = Data_base.data_generation()
        (
            self.data_dummy_train,
            self.data_dummy_test,
        ) = Data_base.data_generation_binary_classification(
            train=self.data_train.values(), test=self.data_test.values()
        )
    ...
    def simulation_full_features(self):
        ...
        columns_train, X_train, Y_train, df = Tools.data_list_unpack_dict(
            self.data_train
        )

        columns_test, X_test, Y_test, de = Tools.data_list_unpack_dict(self.data_test)

        U_train, V_train = Tools.data_list_unpack_dict(self.data_dummy_train)

        U_test, V_test = Tools.data_list_unpack_dict(self.data_dummy_test)
        ...
        (
            ...
            names, exog, endog, x, y, family, method, par_reg, task, mis_classif
    ...
    def simulation_discriminant_methods(self):
        ...
        for cols in cols_list:
            ...
            acc, tpr, tnr, ppv, fpr, fnr, dor, BIAS, pt1, pp1, _, _ = ac.metrics_binary(
                y_t, y_estimated_, False
            )
