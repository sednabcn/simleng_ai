...
class Features_selection(Data_Generation):
        ...
        def simulation_full_features(self):
            ...
            to_model_names=Statsmodels_linear_filter(names,exog,endog,x,y,par,family,method,par_reg,task,mis_classif).\
            statsmodels_linear_supervised()

            Table_results(0,y_calibrated_table,'.3f','fancy_grid','Validation',60).print_table()

            Table_results(0,y_estimated_table,'.3f','fancy_grid','Estimation',60).print_table()
            
            Table_results(0,params_table,'.3f','fancy_grid','Models Parameters',60).print_table()

            
            Draw_numerical_results.frame_from_dict(residuals_table.index,residuals_table,'samples','Residue Pearson','Residuals of solvers on Training Data','Log',' ',' ','square',0)
            ...
            Draw_binary_classification_results(FPR,TPR,to_model_names,params_table,
                                               U_train_exog,V_train,U_test,V_test,y_calibrated_table,y_estimated_table, residuals_table,columns_train,Title,'',).fpn_estimated()
            ...
            Title=" ROC_CURVE "
            Draw_binary_classification_results(FPR,TPR,to_model_names,params_table,\
                                               U_train_exog,V_train,U_test,V_test,y_calibrated_table,y_estimated_table,residuals_table,columns_train,Title,'').roc_curve()
        ...
        def simulation_discriminant_methods(self):
            ...
            for cols in cols_list:
                ...
                acc,tpr,tnr,ppv,fpr,fnr,dor,BIAS,pt1,pp1,_,_=MetricBinaryClassifier.metrics_binary(y_t,y_estimated_,False)
                ...
                acc,tpr,tnr,ppv,fpr,fnr,dor,BIAS,pt1,pp1,_,_=MetricBinaryClassifier.metrics_binary(y_est,y_estimated_,False)
                print('LDA_ESTIMATED')
                #print(acc,tpr,tnr,ppv,fpr,fnr,dor,BIAS,pt1,pp1)
