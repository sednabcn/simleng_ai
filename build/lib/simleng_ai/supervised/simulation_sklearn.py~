...
Normalizer, OneHotEncoder, RobustScaler,\
StandardScaler, add_dummy_feature,\
PolynomialFeatures, binarize, normalize,\
scale, robust_scale, \
...
from sklearn.neighbors import BallTree,NearestCentroid,NearestNeighbors, \
    kneighbors_graph,radius_neighbors_graph,KernelDensity,LocalOutlierFactor,\
    KDTree,KNeighborsClassifier,RadiusNeighborsClassifier,\
...
class Pipeline:
    ...
    def pipeline_classification():
       ...
       Cs=[20,30,50]
       X, y= datasets.make_classification(n_samples=n_samples, n_features=n_features, n_informative=5,random_state=42)
       ...
       enet = Pipeline([
           ('standardscaler', preprocessing.StandardScaler()),
           ('enet', sklm.SGDClassifier(loss="log", penalty="elasticnet",
    ...
    def pipeline_filters():
       ...
       model = Pipeline([('anova', SelectKBest(f_regression, k=3)),('sklm', sklm.LinearRegression())])
       scores = cross_val_score(estimator=model, X=X, y=y, cv=5)
    ...
    def pipeline_regression_cv():
       ...
       X, y, coef = datasets.make_regression(n_samples=n_samples, n_features=n_features, noise=noise_sd, n_informative=5,random_state=42, coef=True)
       ...
       param_grid = {'selectkbest__k':np.arange(10, 110, 10),'ridge__alpha':[.001, .01, .1, 1, 10, 100] }
       t2=time.time()
       ...
       enet_cv = Pipeline([
           ('standardscaler', preprocessing.StandardScaler()),
           ('enet', sklm.ElasticNetCV(max_iter=10000, l1_ratio=l1_ratio, alphas=alphas)),
           ])
       scores = cross_val_score(estimator=enet_cv, X=X, y=y, cv=5)
       t2=time.time()
       print('CPU: %s' % str(t2-t1))
...
#change the name of the models        
    ...
    def fit_regularized(self):
        ...
        if  self.method=='l1' and self.model_name=='smd.LogisticRegression':
                ...
                self.model=sklm.LogisticRegression(self.endog,self.exog).fit_regularized(method='l1',alpha=self.par_reg[0])

        if self.method=='elastic_net' and self.model_name=='sklm.OLS':
                print(self.model_name + " Regularized wwith alpha: %.2f L1wt: %.2f "%(self.par_reg))
                self.model=sklm.OLS(self.endog,self.exog).fit_regularized(method='elastic_net',alpha=self.par_reg[0],L1wt=self.par_reg[1]) 
...
# change the name of models
        ...
        for name in self.names:  
            ...
            bse,z_score=Sklearn_simula.summary_models(self)
