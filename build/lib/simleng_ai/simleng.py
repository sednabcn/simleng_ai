...
class Simleng:
    ...
    def __init__(self,file_input,score=None):
        ...
        _,self.dataset,_,_,self.features,\
                        self.target,self.strategies_client,self.params=self.gen.get_input_info()
        
    def simulation_strategies(self):
        """strategies management in Simleng"""
        self.strategies['Features_selection']=["full_features","features_selection_z_score",\
                                               "K_fold_cross_validation","pca",\
                                               "additional_test","discriminant_methods"]
        self.strategies['Data']=['split','balance','augment','synthetic','unique_rep']
        self.strategies['Training']=['base','full','pre-trained']
        self.strategies['Stochastic']=['boostrap','combined','matrices','operational','meshing',
                                      'variational','app']
        self.strategies['Solver']=['optimizer','parameters','convergence','metrics']

        print(self.strategies_client["STRATEGY"])
        try:
           if self.strategies_client["METHOD"] not in\
              self.strategies[self.strategies_client["STRATEGY"]]:
                       strategies[self.strategies_client["STRATEGY"]].\
                       append(self.strategies_client["METHOD"])
        except:
    ...
    def strategies_features_master(self):            
        ...
        if self.strategies_client["STRATEGY"]=="Features_selection":
            pepe=[self.data_train,self.data_test,self.data_dummy_train,\
            self.data_dummy_test,self.strategies_client["METHOD"],self.params]
    ...
    def switch(self,mlmethod,goal,nclass,strategy):
                   ...
                   if mlmethod=='SUPERV':
                              if goal=="CLASSIFICATION":
                                      if nclass=='2':
                                           self.data_train,self.data_test,_,self.X_train,self.X_test,\
                                           self.y_train,self.y_test,_,_,_,_=\
                                               self.gen.data_generation_functions()
                                           self.data_dummy_train,self.data_dummy_test=self.gen.\
                                           data_generation_target()
