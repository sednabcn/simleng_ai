...
class Classifiers_comparison:
      """Draw the decision boundary of classifiers on the first two PCA components"""
      ...
      def init(self,datasets_names, datasets, classifiers_names,classifiers,Title,idoc):
          self.datasets_names=datasets_names
          self.datasets=datasets
          self.names=classifiers_names
          self.classifiers=classifiers
          self.Title=Title
          self.idoc=idoc
          
      def classifiers_comparison(self):
        
          # inputs
          #dataset_names: List of datasets name
          #datasets["dataset_name"] : Dictionary with keys :train,test
    
          figure = plt.figure(figsize=(27, 9))
          i = 1
          # iterate over datasets
          for name in enumerate(dataset_names):
              # processing dataset
              xtrain,y_train=datasets[name]["train"]
              xtest,y_test=datasets[name]["test"]

              principal=PCA(n_components=2)
              principal.fit(xtrain)
              X_train=principal.transform(xtrain)
              X_test = principal.transform(xtest)
              ...
              ax = plt.subplot(len(names), len(classifiers) + 1, i)
              if ds_cnt == 0:
