class Data_Engineering:
    ...
    from ..resources.output import table, image_to_report
...
class Correlation(Data_Engineering):
    ...
    def correlation_level(self):
        ...
        if LL > 0:
            ...
            return print(
                "The following predictors are highly pair-correlationed:"
                + ",".join(str(name) for name in mcorr_high),
                sep="\n",
            ), print(
                "\n",
                "The following predictors are midly pair-correlationed "
                + ",".join(str(name) for name in mcorr_low),
                sep="\n",
            )
        else:
...
class PCA:
    ...
    def pca(self):
        ...
        order = [
            ii
            for ii, vals in sorted(
                enumerate(np.abs(eigvalues)), key=lambda x: x[1], reverse=True
            )
        ]
    ...
    def pca_draw_major_minor_factors(self, target):
        ...
        major_factor, minor_factor, _, _, _, explained_variance_, _ = PCA(
            self.x, self.ncomp
        ).pca()
        ...
        components = np.array(
            [[major_factor[0], minor_factor[0]], [-minor_factor[0], major_factor[0]]]
        )
    ...
    def pca_show_table(self):
        ...
        eig_data = [
            (self.x.columns[i], np.abs(eigvalues[i]), cum_var_exp[i])
            for i in range(self.ncomp)
        ]

        eig_data = pd.DataFrame(
            eig_data, columns=["Predictors", "Eig.Abs", "cum_var_exp"]
        )
        ...
        return table(
            eig_data, ".2f", "fancy_grid", "Eigenvalues in descending order", 60
        )
    ...
    def pca_draw_by_components(self):
        ...
        _, _, _, eigvalues, eigvectors, var_exp, cum_var_exp = PCA(
            self.x, self.ncomp
        ).pca()
...
class Best_features_filter:
    ...
    def variance_influence_factors(self):
        ...
        vif["VIF Factor"] = [
            variance_inflation_factor(X_vif, i) for i in range(X_vif.shape[1])
        ]
        vif = np.asarray(vif)
        vif = dict(vif)
        vif_inverse = {}
        ...
        keys_ordered = [
            vif_inverse[name] for name in sorted(vif.values(), reverse=True)
        ]
        ...
        key_colinearity = vif_ordered.index[
            np.where(vif_ordered.iloc[:, 0] > self.vif_treshold)
        ]

        return (
            key_colinearity,
            table(vif_ordered, ".3f", "simple", "Variance Influence Factors", 0),
            print(
                "\n",
    ...
    def show_best_pca_predictors(self):
        ...
        order = [
            ii
            for ii, vals in sorted(
                enumerate(np.abs(eigenvalues)), key=lambda x: x[1], reverse=True
            )
        ]
...
class Best_features_wrap:
    ...
    def cross_validation_binary_classification(
        self,
        names,
        cols_data,
        cols_base_validation,
        cols_index,
        train_exog,
        x_test_exog,
        endog,
    ...
    def add_feature_selection(
        ...
        while (Iperfor < Iperfor0) or (len(cols_base) < len(cols_data) + 1):
            ...
            for nn in columns_diff:
                ...
                try:
                    ...
                    columns_Index = add_index_to_list_of_indexes(
                        columns_Index, nn, True
                    )
            ...
            predictor, predictor_list, Imax, PPV, Iperfor = max_in_col_matrix(
                Idata, "ACC", "PPV", Iperfor
            )

            Iperformance_1[N_1] = [
                Iperformance_2[predictor][0],
                Iperformance_2[predictor][1],
                Iperformance_2[predictor][8],
                Iperformance_2[predictor][2],
                Iperformance_2[predictor][4],
                Iperformance_2[predictor][5],
                Iperformance_2[predictor][6],
                Iperformance_2[predictor][7],
            ]
            ...
            try:
                ...
                cols_index = add_index_to_list_of_indexes(
                    cols_index, Iperformance_2[predictor][3], True
                )
        ...
        if K_fold == 1:
            ...
            print(
                "The Best performance is {0:.2f}%".format(Imax) + " using",
                Title,
                sep="\n",
            )
            ...
            return (
                ...
                model_names,
                columns_base,
                y_predicted_table_selected,
                y_estimated_table_selected,
                params_table_selected,
                residuals_table_selected,
            )

        else:
            return Iperformance_1

    def draw_K_fold_numerical_results(
        self, K_fold, N_cols_base, N_features, data_fold, Iperformance
    ):
        ...
        ACC_data = pd.DataFrame(
            index=range(K_fold),
            columns=range(1 + N_cols_base, 1 + N_cols_base + N_features),
        )
        ...
        Table_results(
            ...
            ".2f",
            "fancy_grid",
            "ACC K_fold Cross-Validation in Binary Classification",
            40,
        ).print_table()

        Table_results(
            12, ACC_data_mean, ".2f", "fancy_grid", " ACC Data Mean vs Features", 50
        ).print_table()
        ...
        Table_results(
            ...
            ".2f",
            "fancy_grid",
            "PPV in K_fold Cross-Validation to Binary Classification",
            40,
        ).print_table()
        Table_results(
            13, PPV_data_mean, ".2f", "fancy_grid", " PPV Data Mean vs Features", 50
        ).print_table()
    ...
    def K_fold_full_prediction_results(
        ...
        for ii in predictor_list:
            ...
            Table_results(
                0, params_table, ".3f", "fancy_grid", "Models Parameters", 30
            ).print_table()
    ...
    def K_fold_numerical_results(
        ...
        k_l = list(Iperformance[nn].keys())
        Title = (
            Iperformance[nn][k_l[mm]][3]
            + " with "
            + str(mm + 1 + N_cols_data)
            + " predictors :"
        )
        ...
        Title += (
            ...
            + "splitting in "
            + str(K_fold)
            + " folds and testing the fold number: "
            + str(nn)
        )
        print(
            "\n",
