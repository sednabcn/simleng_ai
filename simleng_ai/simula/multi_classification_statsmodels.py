...
class Multi_classification_statsmodels(Data_Generation):
    def __init__(self, *args):
        ...
        if isinstance(self.nclass,int):
            self.par.append(int(self.nclass))
        
    def multi_classification_statsmodels_master(self):
        print(self.proc)
        # print(self.data_train.values())
        # print(self.data_dummy_train.values())
        method_name = "multi_classification_" + str(self.proc)
        print(method_name)
        process = getattr(self, method_name, "Invalid Method selected")
        return process()

    def multi_classification_full_features(self):
        if self.idoc == 1:
        ...
        names = ["GLM", "Logit", "GenLogit", "Probit","MNLogit","OLS"]
        exog = U_train_exog
        endog = V_train
        x = U_test_exog
        y = V_test
        ...
        if int(self.nclass)==2:
            task = "MultiClassification"
        elif int(self.nclass)>2:
                task= "MultiClassification"
        else:
            pass
        ...
        Table_results(
            0, residuals_table, ".3f", "fancy_grid", "Model Residuals", 60
        ).print_table()
    ...
    def multi_classification_features_z_score(self):
        ...
        if self.idoc >= 1:
            print("Fig:Draw multi classification_results of Best Predictors")
        ...
        Title = (
            "Multi Classification Regions using statsmodels",
        ...
        kind = "Validation"
        Title = "Multi Classification using statsmodels" " (Validation Case)"
        if self.idoc >= 1:
            print("Fig:Draw multi Classification using Statsmodels(Validation Case)")
        Draw_binary_classification_results(
            FPR,
            TPR,
            names,
        ...
        if self.idoc >= 1:
            print("Fig:Multi Classification(Prediction case)")
        kind = "Prediction"
        Title = "Multi Classification using statsmodels" " (Prediction Case)"
        Draw_binary_classification_results(
            FPR,
            TPR,
            names,
    ...
    def multi_classification_K_fold_cross_validation(self):
        ...
        kind = "Validation"
        Title = (
            "K_fold Cross-Validation in Multi Classification using statsmodels"
            " (Validation Case)"
        )
        if self.idoc >= 1:
            print("Fig:Multi classification with K-fold validation")
        Draw_binary_classification_results(
            "",
            "",
            names,
        ...
        if self.idoc >= 1:
            print("Fig:Multi Missclassification case")
        Draw_binary_classification_results(
            "",
            "",
            names,
    ...
    def multi_classification_with_pca(self):
    ...
    def multi_classification_additional_test(self):
        """
    ...
    def multi_classification_discriminant_methods(self):
        columns_train, X_train, Y_train, df = data_list_unpack_dict(self.data_train)
